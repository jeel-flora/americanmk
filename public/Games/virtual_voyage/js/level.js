...
var LevelState = {
    preload: function() {
        settings(this);
        game.stage.backgroundColor = "#95DDEC";
    },
    create: function() {
        sfx = new Object;
        sfx.fxJump = this.add.audio('fxJump', 1.5);
        sfx.fxCrunch = this.add.audio('fxCrunch');
        sfx.fxDD = this.add.audio('fxDD');
        sfx.fxDing = this.add.audio('fxDing');
        sfx.fxBump = this.add.audio('fxBump', 0.5);
        sfx.fxHit = this.add.audio('fxHit');
        world = this;
        world.doneCT = null;
        world.q = null;
        pars = [];
        ...
        sprt.animations.add('run', [0, 1, 2, 3, 4, 5], 10);
        sprt.animations.add('jump', [6], 1);
        sprt.animations.add('fall', [7], 1);
    ...
    update: function() {
        ...
        if (sprt.visible) {
            if (!sprt.dd) {
                ...
                spikeCode();
                saniCode();
                game.camera.focusOnXY(sprt.x, sprt.y - 40);
            }
            logCode();
        }
        world.bg.tilePosition.x = game.camera.x * -0.5;
    },
    render: function() {
        if (rotDevice) {
            rotDevice.x = game.camera.x;
            rotDevice.y = game.camera.y;
            rotDevice.bringToTop();
        }
                ...
                if (!sprt.visible) {
                    ...
                    if (levl > game.ldat) {
                        ...
                        if (game.storageAvailable) {
                            localStorage.setItem('ldatBeavus', game.ldat);
                        }
                    }
                    var nextButton = game.add.button(400, 225, 'btnNextLevel', function() {
                        clickSound();
                        newState();
                    });
                    ...
                    if (levl == 5 || levl == 9 || levl == 13 || levl == 17) {
                        showAd();
                    }
                    return;
                }
...
function addTut() {
    ...
    game.tut = world.game.add.sprite(280, 190, t);
    game.tut.anchor.setTo(0.5, 0);
    game.tut.animations.add('idle', null, 8);
    game.tut.play('idle', null, true);
    game.tut.nr = 1;
    game.world.setChildIndex(game.tut, sprt.z);
}
...
function tutCode() {
    ...
    if (logs.length <= 0 && game.tut.nr) {
        game.tut.destroy();
        game.tut = null;
        game.tut = world.game.add.sprite(world.xit.x, 150, 'tut2');
        game.tut.anchor.setTo(0.5, 0);
        game.tut.animations.add('idle', null, 8);
        game.tut.play('idle', null, true);
        game.world.setChildIndex(game.tut, sprt.z);
    }
    game.tut.visible = sprt.visible;
}
...
function iniLevel() {
    ...
    for (var i = 0; i < platArray.length; i++) {
        var ob = platArray[i];
        var plat = world.game.add.sprite(ob.x, ob.y, 'plat');
        plat.visible = false;
        plat.width = ob.w;
        plat.height = ob.h;
        plat.l = ob.x;
    ...
    txtCode(ldat.txt);
    world.xit = world.game.add.sprite(ldat.xit.x, ldat.xit.y, 'xit');
    world.xit.kolSize = 1;
    ...
    world.xit.animations.add('up', [1, 2, 3, 4, 5], 12);
    world.xit.animations.add('down', [6, 6, 6, 6, 7, 8, 9, 10, 0], 12);
    world.xit.anchor.setTo(0.5, 1);
    world.xit.frame = 0;
    world.xit.sendToBack();
    sprt.position.setTo(ldat.sprt.x, ldat.sprt.y);
    sprt.xx = ldat.sprt.xx;
    iniPause();
    game.world.setBounds(ldat.lim.x, ldat.lim.y, ldat.lim.w, ldat.lim.h);
    world.limR = ldat.lim.x + ldat.lim.w;
    world.limB = ldat.lim.y + ldat.lim.h;
    iniBG();
}
...
function iniPause() {
    ...
    p.btn = [];
    p.pop.create(0, 0, 'pauseBG');
    addButtons('btnHome', 0);
    game.btnMu = addButtons('btnMu', -150);
    game.btnFX = addButtons('btnFX', 150);
    setMuFXBtns();
}

function addButtons(nam, xOff) {
    var b = p.pop.create(game.world.centerX + xOff, game.world.centerY, nam);
...
function iniBG() {
    world.bg = game.add.tileSprite(0, 0, 800, 450, 'bg1');
...
function gravCode() {
    ...
    if (sprt.y > world.limB + 200) {
        sprt.dd = true;
        fxPlay(sfx.fxHit);
        game.camera.follow(null);
        world.doneCT = 1;
        sprt.camX = game.camera.x;
        sprt.camX = game.camera.y;
        world.q = 25;
...
function bumperCode() {
    ...
    for (var i = 0; i < bumpers.length; i++) {
        var b = bumpers[i];
        if (sprt.yy < 0 || sprt.y - sprt.yy > b.y - 70) {
            continue;
        }
        if (sprtHitTest(b)) {
            bmp = b;
            if (sprt.x >= b.x - 50 && sprt.x <= b.x + 50) {
                break;
            }
        }
    }
...
function xitCode() {
    if (world.xit.frame == 5) {
        if (!sprt.ju && sprtHitTest(world.xit)) {
            if (sprt.x < world.xit.x - 5 || sprt.x > world.xit.x + 5) {
                return;
            }
            fxPlay(sfx.fxDing);
            world.xit.play('down', null, false);
            sprt.visible = false;
            world.doneCT = 1;
            p.visible = false;
        }
...
function quakeCode() {
    if (world.q) {
        game.camera.y = sprt.camX - Math.random() * world.q - world.q / 2;
        world.q -= 0.5;
        if (world.q < 0) {
            world.q = null;
        }
